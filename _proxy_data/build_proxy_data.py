...
def _cal_acc(dataset, net, batch=256):
    ...
    def infer(net, test_loader, criterion):
        ...
        top1_avg = utils.AverageMeter()
        top5_avg = utils.AverageMeter()
        with torch.no_grad():
            for data, target in test_loader:
                ...
                top1_avg.update(acc1[0], n)
                top5_avg.update(acc5[0], n)
        return loss_avg.avg, top1_avg.avg, top5_avg.avg
    ...
    loss, top1_acc, top5_acc = infer(net, test_loader, criterion)
    print('loss, top1_acc, top5_acc', loss, top1_acc, top5_acc)
